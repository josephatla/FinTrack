...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold fs-2 m-0">{{ __('dashboard.transaction_history') }}</h2>
    
    <button class="btn btn-outline-primary" type="button" data-bs-toggle="collapse" data-bs-target="#filterCollapse" aria-expanded="false" aria-controls="filterCollapse" title="{{ __('transactions.filter_transactions') ?? 'Filter' }}">
...
<div class="collapse {{ request()->hasAny(['search', 'account', 'type', 'category', 'month', 'year']) ? 'show' : '' }}" id="filterCollapse">
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-light fw-bold">
            {{ __('transactions.filter_transactions') }}
        </div>
        <div class="card-body">
            <form method="GET" action="{{ url()->current() }}" class="row g-3 align-items-end">
                ...
                <div class="col-12 d-flex gap-2 pt-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-check me-1"></i> {{ __('transactions.apply_filter') }}
                    </button>
                    
                    @if (request()->hasAny(['search', 'account', 'type', 'category', 'month', 'year']))
